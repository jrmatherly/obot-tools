...
```bash
# In obot-tools repository (for standalone testing)
go run .

# From obot-entraid repository (for integration testing)
export OBOT_SERVER_TOOL_REGISTRIES="github.com/jrmatherly/obot-tools"
make dev
```

This ensures Obot uses your fork's tool registry with the custom auth providers.
